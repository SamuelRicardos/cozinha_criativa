...
if (typeof receita.ingredientes === 'string') {
    ...
    ingredientes = receita.ingredientes.split(/(?<=\.)\s*/);
} else if (Array.isArray(receita.ingredientes)) {
    ingredientes = receita.ingredientes;
} else {
    ingredientes = []; // Caso não seja string ou array, define um array vazio
}
...
ingredientes.forEach((ingrediente) => {
    ...
    doc.text(`- ${ingrediente.trim()}`, margins.left + 10, y); // Usa trim() para remover espaços extras
    y += 10;
});
